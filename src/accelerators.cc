...
void accel_map_load_merged(void)
{
	g_autoptr(GKeyFile) defaults = g_key_file_new();
	//~ g_autoptr(GKeyFile) default = g_key_file_new();
	...
	//~ accels_merged = defaults;

gsize length;
gchar *text;

text = g_key_file_to_data(defaults, &length, NULL);

g_print("%s\n", text);
text = g_key_file_to_data(user, &length, NULL);

g_print("%s\n", text);
text = g_key_file_to_data(accels_merged, &length, NULL);
