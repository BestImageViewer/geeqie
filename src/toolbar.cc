									...
									const gchar *stock_id, GtkBox *box)
{
	...
	if (stock_id)
		...
		if (pixbuf)
			...
			{
			image = gtk_image_new_from_icon_name(get_icon_for_action_name(name), GTK_ICON_SIZE_BUTTON);
			}
		}
	else
		{
...
{
	ToolbarType bar = static_cast<ToolbarType>(GPOINTER_TO_INT(data));
	GMenu *menu_model;
	GMenu *main_submenu;
...
GtkWidget *toolbar_select_new(LayoutWindow *lw, ToolbarType bar)
{
	...
	GtkWidget *add_button = pref_toolbar_button(tbar, GQ_ICON_ADD, _("Add"), FALSE,
	                        _("Add Toolbar Item"),
	                        G_CALLBACK(toolbar_menu_add_cb), GINT_TO_POINTER(bar));
	gtk_widget_show(add_button);

	GApplication *app = g_application_get_default();
	if (bar == TOOLBAR_MAIN)
		{
		register_actions_from_table(GTK_APPLICATION(app), get_config_window(), toolbar_main_actions, get_keyfile_merged(), widget);
		}
	else
		{
