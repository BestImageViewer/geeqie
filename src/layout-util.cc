...
const char *get_icon_for_action_name(const char *accel);
...
static bool layout_key_press_cb(GtkWidget *widget, GdkEventKey *event, gpointer)
{
...
void layout_return_cb(GSimpleAction *, GVariant *, gpointer)
{
	auto lw = get_current_layout();
...
static void layout_menu_exif_rotate_cb(GSimpleAction *action, GVariant *, gpointer)
{
	auto lw = get_current_layout();

	bool active = g_variant_get_boolean(g_action_get_state(G_ACTION(action)));
...
static void layout_menu_select_rectangle_cb(GSimpleAction *action, GVariant *, gpointer)
{
	bool active = g_variant_get_boolean(g_action_get_state(G_ACTION(action)));
...
static void layout_menu_split_pane_sync_cb(GSimpleAction *action,
                                           GVariant      *parameter,
                                           gpointer       data)
{
...
static void layout_menu_list_cb(GSimpleAction *action, GVariant *state, gpointer)
{
...
static void layout_menu_overlay_cb(GSimpleAction *action, GVariant *state, gpointer)
{
	auto lw = get_current_layout();
...
static void layout_menu_histogram_cb(GSimpleAction *action, GVariant * , gpointer)
{
	auto lw = get_current_layout();


	bool active2 = g_variant_get_boolean(g_action_get_state(G_ACTION(action)));

	g_action_change_state(G_ACTION(action), g_variant_new_boolean(!active2));
...
static void layout_menu_stereo_mode_cb(GSimpleAction *, GVariant *state, gpointer data)
{
	auto lw = static_cast<LayoutWindow *>(data);
	const char *value = g_variant_get_string(state, nullptr);
...
static void layout_color_menu_use_image_cb(GSimpleAction *action, GVariant *state, gpointer)
{
	auto lw = get_current_layout();
	gint input;
	gboolean use_image;
...
static void layout_color_menu_input_cb(GSimpleAction *, GVariant *, gpointer)
{
...
//~ static void register_plugin_run_action(GtkWidget *window, GHashTable *by_key)
//~ {
    //~ GSimpleAction *a = g_simple_action_new("plugin-run", G_VARIANT_TYPE_STRING);
    //~ g_signal_connect(a, "activate", G_CALLBACK(plugin_run_cb), by_key);
...
static void layout_actions_setup_editors(LayoutWindow *lw)
{
	...
	plugins_menu_populate(plugins_menu, "win.plugin-run");
...
int find_section_by_label (GMenuModel *model, const gchar )
{
...
int find_item_by_label (GMenuModel *model, const char *label)
{
	int n = g_menu_model_get_n_items (model);

	for (int i = 0; i < n; i++)
		{
		const char *item_action = NULL;
...
void layout_actions_setup(LayoutWindow   )
{
...
/* Optional: add a target */

/* 4. Append it to the section */
...
void layout_toolbar_add(LayoutWindow *lw, ToolbarType type, const gchar *action_name)
{
	...
	if (g_strcmp0(action_name, "Separator") == 0)
		...
		if (button)
			...
			GtkWidget *image = gtk_image_new_from_icon_name(get_icon_for_action_name(action_name), GTK_ICON_SIZE_BUTTON);
...
GStrv get_tooltips(void)
{
	...
	for (unsigned int i = 0; i < G_N_ELEMENTS(app_actions); i++)
		{
		if (app_actions[i].description)
			{
			g_ptr_array_add(array, g_strdup(app_actions[i].description));
			}
		}
