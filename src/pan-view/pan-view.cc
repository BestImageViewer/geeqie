...
static void pan_exif_date_toggle_cb(GSimpleAction *action, GVariant *, gpointer data)
{
	auto pw = static_cast<PanWindow *>(data);
...
static void pan_info_toggle_exif_cb(GSimpleAction *action, GVariant *, gpointer data)
{
	auto pw = static_cast<PanWindow *>(data);
	bool active = g_variant_get_boolean(g_action_get_state(G_ACTION(action)));
	g_action_change_state(G_ACTION(action), g_variant_new_boolean(!active));
...
static void pan_popup_menu(PanWindow *pw)
{
	...
	plugins_menu_populate(plugins_menu, "win.pan-win-plugin-run");
	...
	popup_menu(menu_model, pw->window);
}
